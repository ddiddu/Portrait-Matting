...
class HRBranch(nn.Module):
    ...
    def forward(self, img, enc4x, lr8x):
        img4x = F.interpolate(img, scale_factor=1/4, mode='bilinear', align_corners=False) # torch.Size([1, 3, 128, 128])

        enc4x = self.tohr_enc4x(enc4x) # torch.Size([1, 32, 128, 128])
        hr4x = self.tohr_hr4x(enc4x)

        lr4x = F.interpolate(lr8x, scale_factor=2, mode='bilinear', align_corners=False) # torch.Size([1, 32, 128, 128])
        hr4x = self.tohr_lr4x(torch.cat((hr4x, lr4x, img4x), dim=1)) # torch.Size([1, 32, 128, 128])
...
class MODNet(nn.Module):
    ...
    def __init__(self, in_channels=3, hr_channels=32, backbone_arch='mobilenetv2', backbone_pretrained=True, only_semantic=True):
