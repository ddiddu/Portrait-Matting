...
#         self.backbone = backbone
        # self.mnv3_se_block = SEBlock(enc_channels[4], enc_channels[4], reduction=4)
        # self.mnv3_conv_lr16x = Conv2dIBNormRelu(enc_channels[4], enc_channels[3], 5, stride=1, padding=2) # (1280, 96, 5, 1, 2)
        ...
        self.mnv3_se_block = SEBlock(576, 576, reduction=4)
        self.mnv3_conv_lr16x = Conv2dIBNormRelu(576, enc_channels[3], 5, stride=1, padding=2) # (1280, 96, 5, 1, 2)


    def forward(self, img, inference):
        # enc_features = self.backbone.forward(img)
        enc_features = self.mobilenetv3.forward(img)
        # enc2x, enc4x, enc8x, enc32x = enc_features[0], enc_features[1], enc_features[2], enc_features[4]
        # torch.Size([1, 16, 128, 128]) torch.Size([1, 24, 64, 64]) torch.Size([1, 40, 32, 32]) torch.Size([1, 48, 32, 32]) torch.Size([1, 96, 16, 16]) torch.Size([1, 576, 16, 16]) torch.Size([1, 1280, 1, 1])
        enc2x, enc4x, enc64x = enc_features[0], enc_features[1], enc_features[5]
        enc2x = F.interpolate(enc2x, scale_factor=2, mode='bilinear', align_corners=False)
        enc4x = F.interpolate(enc4x, scale_factor=2, mode='bilinear', align_corners=False)

        enc64x = self.mnv3_se_block(enc64x) # [1, 1280, 8, 8]
        lr16x = F.interpolate(enc64x, scale_factor=2, mode='bilinear', align_corners=False)
        lr16x = self.mnv3_conv_lr16x(lr16x) # [1, 96, 16, 16]
...
class MODNet_MNV3(nn.Module):
    ...
    def __init__(self, in_channels=3, hr_channels=32, backbone_arch='mobilenetv3', backbone_pretrained=True):
        ...
        self.f_branch = FusionBranch(self.hr_channels, self.mobilenetv3.enc_channels)
