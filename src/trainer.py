# cSpell:ignore blurer cuda trimap optim ckpt numpy scipy ndimage splitly  
...
def fine_tuning_iter(
    modnet, optimizer, image, matte,
    fine_semantic_scale=100.0, 
    validation=False):
    """ Self-Supervised sub-objective consistency (SOC) adaptation iteration of MODNet
    This function fine-tunes MODNet for one interation in an unlabeled dataset.
    Modified original SOC adaptation iteration in purpose to improve boundary segmentation in S block.
    ...
    Arguments:
        ...
        image (torch.autograd.Variable): input RGB image
                                         its pixel values should be normalized
        fine_semantic_scale (float): scale of fine semantic loss
        fine_detail_scale (float): scale of fine detail loss
        
    Returns:
        fine_semantic_loss (torch.Tensor): loss of the semantic and matte
        fine_detail_loss (torch.Tensor): loss of the detail and semantic
    ...
    Example:
        ...
        import torch
        from src.models.modnet import MODNet
        from src.trainer import fine_tuning_iter
        bs = 1          # batch size
        lr = 0.00001    # learn rate
        epochs = 10     # total epochs
        modnet = torch.nn.DataParallel(MODNet()).cuda()
        ...
        for epoch in range(0, epochs):
            for idx, (image) in enumerate(dataloader):
                fine_semantic_loss, fine_detail_loss = fine_tuning_iter(modnet, backup_modnet, optimizer, image)
    
    model.eval():
        notify all your layers that you are in eval mode
        batchnorm or dropout layers will work in eval mode instead of training mode
        batchnorm 과 dropout layer은 학습 시에는 동작해야 하지만, inference 시에는 동작하지 않음

    torch.no_grad():
        impacts the autograd engine and deactivate it
        reduce memory usuage and speed up computations but you won't be able to backprop
        autograd을 끔으로써 메모리 사용량을 줄이고 연산 속도를 높임
    ...
    if validation == False:
        # set the main model to train mode and freeze layers
        modnet.train()
        for params in modnet.parameters():
            params.requires_grad = False
        
        # select layers for fine tuning and freeze its norm layers
        modnet.module.unfreeze_semantic() # requires_grad = True
        modnet.module.freeze_norm() # Batchnorm2d.eval(), InstanceNorm2d.eval()

        # clear the optimizer
        optimizer.zero_grad()

        # fine semantic loss between 'pred_matte' and 'pred_backup_matte'
        # generate pseudo ground truth for 'pred_matte'
        pseudo_gt_matte = matte * (matte > 0.01).float()

        # forward the main model
        pred_matte = modnet(image, False)

        # calculate the fine semantic loss
        fine_semantic_loss = F.l1_loss(pred_matte, pseudo_gt_matte)
        fine_semantic_loss = torch.mean(fine_semantic_loss)
        
        fine_semantic_loss = fine_semantic_scale * fine_semantic_loss
        
        loss = fine_semantic_loss
        ...
        # set the main model to eval mode
        modnet.eval()
        ...
        # CUDA error: out of memory: https://gaussian37.github.io/dl-pytorch-snippets/
        loss = torch.nn.MSELoss()
        fine_semantic_loss = loss(pred_matte, pseudo_gt_matte.cuda()).detach()
        mse = torch.mean(fine_semantic_loss)
        
        fine_semantic_loss = F.l1_loss(pred_matte, pseudo_gt_matte.cuda()).detach()
        mad = torch.mean(fine_semantic_loss)

        return mse, mad


def soc_mnv3_iter(
    modnet, backup_modnet, optimizer, image, matte=None,
    soc_semantic_scale=10.0, soc_detail_scale=10.0, soc_matte_scale=1.0):
    ...
    if matte == None:
        ...
        # forward the backup model
        with torch.no_grad():
            pred_backup_semantic, pred_backup_detail, pred_backup_matte = backup_modnet(image, False)

        # calculate the boundary mask from `pred_backup_matte` and `pred_backup_semantic`
        pred_matte_fg = (pred_backup_matte.detach() > 0.1).float()
        pred_semantic_fg = (pred_backup_semantic.detach() > 0.1).float()
        pred_semantic_fg = F.interpolate(pred_semantic_fg, scale_factor=16, mode='bilinear')
        pred_fg = pred_matte_fg * pred_semantic_fg

        n, c, h, w = pred_backup_matte.shape
        np_pred_fg = pred_fg.data.cpu().numpy()
        np_boundaries = np.zeros([n, c, h, w])
        for sdx in range(0, n):
            sample_np_boundaries = np_boundaries[sdx, 0, ...]
        ...
        # sub-objectives consistency between `pred_semantic` and `pred_backup_matte`
        # generate pseudo ground truth for `pred_semantic`
        downsampled_pred_matte = blurer(F.interpolate(pred_backup_matte, scale_factor=1/16, mode='bilinear'))
        pseudo_gt_semantic = downsampled_pred_matte.detach()
        pseudo_gt_semantic = pseudo_gt_semantic * (pseudo_gt_semantic > 0.01).float()

        # calculate the SOC semantic loss
        soc_semantic_loss = F.mse_loss(pred_semantic, pseudo_gt_semantic)
        soc_semantic_loss = soc_semantic_scale * torch.mean(soc_semantic_loss)
        ...
        # generate pseudo ground truth for `pred_matte`
        pseudo_gt_matte = pred_backup_semantic.detach()
        pseudo_gt_matte = pseudo_gt_matte * (pseudo_gt_matte > 0.01).float()

        # calculate the SOC matte loss
        soc_matte_loss = F.mse_loss(downsampled_pred_matte, pseudo_gt_matte)
        soc_matte_loss = soc_matte_scale * torch.mean(soc_matte_loss)

        # calculate the final loss, backward the loss, and update the model 
        loss = soc_semantic_loss + soc_detail_loss + soc_matte_loss

        loss.backward()
        optimizer.step()

        return soc_semantic_loss, soc_detail_loss, soc_matte_loss
        ...
        pseudo_gt_matte = matte * (matte > 0.01).float()
        _, _, pred_matte = modnet(image, False)
